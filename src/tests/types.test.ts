...
describe('Export Types (v0.1.0)', () => {
	describe('ExportMode type', () => {
		it('accepts quick and bundled values', () => {
			...
			const modes: import('$lib/types').ExportMode[] = ['quick', 'bundled'];
	...
	describe('ExportMetadata interface', () => {
		it('accepts valid metadata structure', async () => {
			...
			expect(metadata.deviceCount).toBe(10);
			expect(metadata.sourceIncluded).toBe(true);
		});
	});

	describe('BundledExportOptions interface', () => {
		it('extends ExportOptions with includeSource', async () => {
			const options: import('$lib/types').BundledExportOptions = {
				format: 'png',
				scope: 'all',
				includeNames: true,
				includeLegend: false,
			...
			expect(options.exportMode).toBe('bundled');
			expect(options.includeSource).toBe(true);
		});

		it('allows includeSource to be false', async () => {
			const options: import('$lib/types').BundledExportOptions = {
				format: 'jpeg',
				scope: 'selected',
				includeNames: false,
			...
			expect(options.includeSource).toBe(false);
		});
	});
});

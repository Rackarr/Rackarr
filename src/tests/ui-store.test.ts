...
import { getUIStore, resetUIStore, ZOOM_MIN, ZOOM_MAX, ZOOM_STEP } from '$lib/stores/ui.svelte';
...
describe('UI Store', () => {
	...
	describe('Theme', () => {
		...
		it('theme change persists to localStorage', () => {
			const store = getUIStore();
			store.setTheme('light');
			expect(localStorageMock.setItem).toHaveBeenCalledWith('rackarr_theme', 'light');
		});

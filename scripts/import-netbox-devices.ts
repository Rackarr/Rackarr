...
const NETBOX_API_BASE = 'https://api.github.com/repos/netbox-community/devicetype-library/contents';
...
async function fetchJson<T>(url: string): Promise<T> {
	const response = await fetch(url, {
		headers: {
			...
			'User-Agent': 'Rackarr-Import-Script'
		}
	});
...
async function listVendorDevices(vendor: string): Promise<string[]> {
	...
	return files.filter((f) => f.name.endsWith('.yaml')).map((f) => f.name.replace('.yaml', ''));
}

async function fetchDeviceYaml(vendor: string, slug: string): Promise<NetBoxDevice | null> {
...
function inferCategory(device: NetBoxDevice): string {
	...
	if (model.includes('nas') || model.includes('rs') || model.includes('ds')) return 'storage';
	if (model.includes('poweredge') || model.includes('proliant')) return 'server';
...
): Promise<{ device: NetBoxDevice | null; frontImage: boolean; rearImage: boolean }> {
	console.log(`\nImporting: ${slug}`);
...
async function main(): Promise<void> {
	...
	if (importedDevices.length > 0 && !options.dryRun && !options.imagesOnly) {
		...
		console.log(`export const ${options.vendor.toLowerCase()}Devices: DeviceType[] = [`);
		importedDevices.forEach((device, i) => {
			console.log(deviceToTypeScript(device) + (i < importedDevices.length - 1 ? ',' : ''));
		});
		console.log(`];`);

		// Generate bundledImages.ts entries
		const devicesWithImages = importedDevices.filter((d) => d.front_image || d.rear_image);

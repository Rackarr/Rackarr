...
test.describe('Starter Library', () => {
	...
	test('device palette is visible and contains starter library devices', async ({ page }) => {
	...
	test('all 12 categories are represented in the palette', async ({ page }) => {
		...
		await expect(
			page.getByRole('listitem', { name: '48-Port Switch, 1U network', exact: true })
		).toBeVisible();
		await expect(
			page.getByRole('listitem', { name: '1U Router/Firewall, 1U network', exact: true })
		).toBeVisible();

		// Patch Panel category (2 items)
		await expect(
			page.getByRole('listitem', { name: '24-Port Patch Panel, 1U patch-panel', exact: true })
		).toBeVisible();
		await expect(
			page.getByRole('listitem', { name: '48-Port Patch Panel, 2U patch-panel', exact: true })
		).toBeVisible();

		// Storage category (3 items)
		await expect(
			page.getByRole('listitem', { name: '1U Storage, 1U storage', exact: true })
		).toBeVisible();
		await expect(
			page.getByRole('listitem', { name: '2U Storage, 2U storage', exact: true })
		).toBeVisible();
		await expect(
			page.getByRole('listitem', { name: '4U Storage, 4U storage', exact: true })
		).toBeVisible();
		...
		await expect(page.getByRole('listitem', { name: '1U KVM, 1U kvm', exact: true })).toBeVisible();
		await expect(
			page.getByRole('listitem', { name: '1U Console Drawer, 1U kvm', exact: true })
		).toBeVisible();

		// AV/Media category (2 items)
		await expect(
			page.getByRole('listitem', { name: '1U Receiver, 1U av-media', exact: true })
		).toBeVisible();
		await expect(
			page.getByRole('listitem', { name: '2U Amplifier, 2U av-media', exact: true })
		).toBeVisible();

		// Cooling category (1 item)
		await expect(
			page.getByRole('listitem', { name: '1U Fan Panel, 1U cooling', exact: true })
		).toBeVisible();

		// Blank category (3 items)
		await expect(
			page.getByRole('listitem', { name: '0.5U Blank, 0.5U blank', exact: true })
		).toBeVisible();
		await expect(
		...
		await expect(
			page.getByRole('listitem', { name: '1U Brush Panel, 1U cable-management', exact: true })
		).toBeVisible();
		await expect(
			page.getByRole('listitem', { name: '1U Cable Management, 1U cable-management', exact: true })
		).toBeVisible();
	});

	test('removed items are NOT present', async ({ page }) => {
		// These items were removed from the library
		await expect(page.locator('.device-palette-item:has-text("1U Generic")')).not.toBeVisible();
		await expect(page.locator('.device-palette-item:has-text("2U Generic")')).not.toBeVisible();
		await expect(page.locator('.device-palette-item:has-text("4U Shelf")')).not.toBeVisible();
		await expect(
			page.locator('.device-palette-item:has-text("0.5U Blanking Fan")')
		).not.toBeVisible();

		// Old names that were renamed
		await expect(page.locator('.device-palette-item:has-text("1U Switch")')).not.toBeVisible();
		await expect(page.locator('.device-palette-item:has-text("1U Patch Panel")')).not.toBeVisible();
		await expect(page.locator('.device-palette-item:has-text("2U Patch Panel")')).not.toBeVisible();
	...
	test('can search for brush panel', async ({ page }) => {
		...
		await expect(page.locator('.device-palette-item:has-text("1U Brush Panel")')).toBeVisible();
	});
	...
	test('can drag 24-Port Switch from palette to rack', async ({ page }) => {
		...
		const deviceItem = page.locator('.device-palette-item:has-text("24-Port Switch")');
		await expect(deviceItem).toBeVisible();

		// Drag device to rack using JavaScript simulation
		await page.evaluate(() => {
			const deviceItem = Array.from(document.querySelectorAll('.device-palette-item')).find((el) =>
				el.textContent?.includes('24-Port Switch')
			);
	...
	test('cable management category has Steel Blue color', async ({ page }) => {
		...
		const cableMgmtItem = page.locator('.device-palette-item:has-text("1U Cable Management")');
		...
		const colorIndicator = cableMgmtItem.locator('.device-preview rect, .category-colour');
		if ((await colorIndicator.count()) > 0) {
			...
			expect(
				fill === '#4682B4' || fill?.toLowerCase() === '#4682b4' || bgColor.includes('70, 130, 180') // RGB for Steel Blue
			).toBeTruthy();
		}
	});
});

...
This audit identifies inefficiencies in the Rackarr development workflow that contribute to excessive resource consumption, particularly Claude API tokens and CI/CD execution time. The primary consumers are:
...
```yaml
steps:
  - run: docker pull ghcr.io/rackarr/rackarr:latest
